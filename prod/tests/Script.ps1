Describe "Verify resource creation" {
    ...
    BeforeAll {
        $resourceGroup = "azchallenge-staleb-prod-rg"
        $webAppExpected = [PSCustomObject]@{
            Name            = 'calicot-app-service-prod'
            ServicePlanName = 'calicot-service-plan-prod'
            Tier            = 'D1'
            OS              = 'Windows'
        ...
        $functionExpected = [PSCustomObject]@{
            ...
            ServicePlanName = 'calicot-azure-functions-app-service-prod'
            ConsumptionTier = 'Y1'
            Location        = 'Canada Central'
        }
        ...
        $serviceBusExpected = [PSCustomObject]@{
            ...
            ServicebusNamespace = 'calicot-servicebus-namespace-prod'
            Tier                = 'Basic'
            Location            = 'Canada Central'
        }
        $sqlDatabaseExpected = [PSCustomObject]@{
            Name          = 'calicot-sql-database-prod'
            SqlServerName = 'calicot-sql-server-prod'
            Edition       = 'Basic'
            Location      = 'canadacentral'
        }
    }
